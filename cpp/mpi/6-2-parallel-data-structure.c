...
// Program implements the non-blocking point-to-point routines instead
// of the blocking routines. MPI_Send and MPI_Recv routines replaced
// with MPI_Isend and MPI_Irecv, and used MPI_Wait or MPI_Waitall
// to test for completion of the nonblocking operations.
...
int main(int argc, char** argv) {
    ...
    int n_req;
    MPI_Request requests[expected_n_procs];
    MPI_Status statuses[expected_n_procs];
    ...
    n_req = 0;
    if (rank < n_procs - 1) {
        MPI_Isend(x_local[max_n / n_procs], max_n, MPI_DOUBLE,
            rank + 1, 0, MPI_COMM_WORLD, &requests[n_req++]);
    }

    if (rank > 0) {
        MPI_Irecv(x_local[0], max_n, MPI_DOUBLE,
            rank - 1, 0, MPI_COMM_WORLD, &requests[n_req++]);
    }

    // Send down unless I'm at the bottom 
    if (rank > 0) {
        MPI_Isend(x_local[1], max_n, MPI_DOUBLE,
            rank - 1, 1, MPI_COMM_WORLD, &requests[n_req++]);
    }

    if (rank < n_procs - 1) {
        MPI_Irecv(x_local[max_n / n_procs + 1], max_n, MPI_DOUBLE,
            rank + 1, 1, MPI_COMM_WORLD, &requests[n_req++]);
    }

    MPI_Waitall(n_req, requests, statuses);
