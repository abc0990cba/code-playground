// This programm use MPI_Sendrecv to exchange data with the neighboring
// processors. That is, processors 0 and 1 exchange, 2 and 3 exchange, etc.
// Then 1 and 2 exchange, 3 and 4, etc. This "head-to-head" exchange may
// be more efficient on some systems.
// https://www.mcs.anl.gov/research/projects/mpi/tutorial/mpiexmpl/src/exchange/C/exchange/main.html
...
int main(int argc, char** argv) {
    ...
    if ((rank % 2) == 0) {
        ...
        MPI_Sendrecv(x_local[max_n / n_procs], max_n, MPI_DOUBLE, up_nbr, 0,
                     x_local[max_n / n_procs + 1], max_n, MPI_DOUBLE, up_nbr, 0,
                     MPI_COMM_WORLD, &status);
    } else {
        // Exchange down
        MPI_Sendrecv(x_local[1], max_n, MPI_DOUBLE, down_nbr, 0,
