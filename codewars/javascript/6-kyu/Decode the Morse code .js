...
// "dots" and "dashes".
// For example, the letter A is coded as ·−,
// letter Q is coded as −−·−,
// and digit 1 is coded as ·−−−−.
// The Morse code is case-insensitive, traditionally capital
// letters are used.
// When the message is written in Morse code, a single space
// is used to separate the character codes and 3 spaces
...
// please keep that in mind. This is mostly because otherwise
// the engine would simply ignore the tests,
// resulting in a "valid" solution.


// Solution 1.
function decodeMorse1(morseCode) {
    const SYMBOL_SEPARATOR = ' ';
    const WORD_SEPARATOR = '   ';

    const decodeMorseLetter = (letter) => MORSE_CODE[letter];
    const decodeMorseWord = (word) =>
        word.split(SYMBOL_SEPARATOR).map(decodeMorseLetter).join('');
